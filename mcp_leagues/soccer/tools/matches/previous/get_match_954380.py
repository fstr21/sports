#!/usr/bin/env python3
"""
SoccerDataAPI Match Details Fetcher for Match ID 954380
Retrieves detailed match information for Chelsea vs Crystal Palace and exports to JSON file
"""
...
def display_match_details(match_data, match_id):
    ...
    if match_data:
        ...
        print(f"Minute: {match_data.get('minute', 'N/A')}")
        ...
        if events:
            ...
            for i, event in enumerate(events, 1):
                ...
                # Handle player_in and player_out for substitutions
                player_in = event.get('player_in', {})
                player_out = event.get('player_out', {})
                
                if event_type == 'substitution':
                    in_name = player_in.get('name', 'N/A')
                    out_name = player_out.get('name', 'N/A')
                    event_desc = f"{minute}' Substitution - {team} - {in_name} IN, {out_name} OUT"
                else:
        ...
        if odds:
            ...
            if handicap:
                ...
                home_hcp = handicap.get('home', 'N/A')
                away_hcp = handicap.get('away', 'N/A')
                print(f"Handicap {market} - Home: {home_hcp}, Away: {away_hcp}")
        ...
        if lineups and lineups.get('lineups'):
            ...
            if formation:
                home_formation = formation.get('home', 'N/A')
                away_formation = formation.get('away', 'N/A')
                print(f"Formations - Home: {home_formation}, Away: {away_formation}")
            
            if home_lineup:
                print(f"\nHome Team ({len(home_lineup)} players):")
                for player in home_lineup:
                    player_info = player.get('player', {})
                    position = player.get('position', 'N/A')
                    name = player_info.get('name', 'N/A')
                    player_id = player_info.get('id', 'N/A')
                    print(f"  {name} ({position}) - ID: {player_id}")
            
            if away_lineup:
                print(f"\nAway Team ({len(away_lineup)} players):")
            ...
            # Bench
            bench = lineups.get('bench', {})
            if bench:
                home_bench = bench.get('home', [])
                away_bench = bench.get('away', [])
                
                if home_bench:
                    print(f"\nHome Team Bench ({len(home_bench)} players):")
                    for player in home_bench[:5]:  # Show first 5
                        player_info = player.get('player', {})
                        position = player.get('position', 'N/A')
                        name = player_info.get('name', 'N/A')
                        print(f"  {name} ({position})")
                    if len(home_bench) > 5:
                        print(f"  ... and {len(home_bench) - 5} more")
                
                if away_bench:
                    print(f"\nAway Team Bench ({len(away_bench)} players):")
                    for player in away_bench[:5]:  # Show first 5
                        player_info = player.get('player', {})
                        position = player.get('position', 'N/A')
                        name = player_info.get('name', 'N/A')
                        print(f"  {name} ({position})")
                    if len(away_bench) > 5:
                        print(f"  ... and {len(away_bench) - 5} more")
...
def main():
    # Configuration
    match_id = 954380  # Chelsea vs Crystal Palace from 17-08-2025
    ...
    print(f"Expected: Chelsea vs Crystal Palace (17/08/2025)")

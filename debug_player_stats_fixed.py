...
class ESPNAPIDebugger:
            ...
            async with self.session.get(url) as response:
                ...
                if status == 200:
                    try:
                        ...
                        self.log(f"[OK] Success: {description} ({status})", {
                            "status": status,
                        ...
                        self.log(f"[ERR] JSON Error: {description}", {"status": status, "json_error": str(e), "text_preview": text[:200]})
                        return {"ok": False, "error": f"JSON decode error: {e}", "status": status}
                else:
                    self.log(f"[ERR] HTTP Error: {description}", {"status": status, "text": text[:200]})
                    return {"ok": False, "error": f"HTTP {status}: {text[:200]}", "status": status}
                    
        except Exception as e:
            self.log(f"[ERR] Request Error: {description}", {"error": str(e)})
    ...
    async def analyze_eventlog_structure(self, player_id: str, player_name: str):
        ...
        self.log(f"\n[ANALYZE] ANALYZING EVENTLOG STRUCTURE FOR {player_name} (ID: {player_id})")
        ...
        self.log(f"[SUMMARY] Eventlog Summary", {
        ...
        for page in range(1, min(total_pages + 1, 4)):  # Limit to first 3 pages for debugging
            ...
            self.log(f"[PAGE] Page {page} Analysis", {
                    ...
                    if event_resp["ok"]:
                        ...
                        if game_date:
                            try:
                                ...
                                self.log(f"    [GAME] Game: {date_str}", game_info)
                                
                            except Exception as e:
                                self.log(f"    [ERR] Date parsing error: {e}")
        
        # Step 4: Sort games by date and analyze coverage
        all_games.sort(key=lambda x: x["date"], reverse=True)
        
        self.log(f"\n[FINAL] FINAL ANALYSIS FOR {player_name}", {
    ...
    async def test_alternative_approaches(self, player_id: str, player_name: str):
        ...
        self.log(f"\n[ALT] TESTING ALTERNATIVE APPROACHES FOR {player_name}")
        ...
        for i, url in enumerate(alt_urls):
            ...
            if resp["ok"]:
                data = resp["data"]
                self.log(f"[OK] Alternative {i+1} worked", {
